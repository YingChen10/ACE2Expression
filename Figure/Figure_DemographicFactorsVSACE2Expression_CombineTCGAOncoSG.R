...
## smoke 
#setwd("~/projects/ACE2/FPKM-UQ_processTCGA2")
all <- read.table("all_expr_clinical.txt")
head(all[,1:6])
#exposure <- read.table("~/projects/ACE2/FPKM-UQ/note/cart/clinical/exposure.tsv",sep="\t",header=T)

smo_his <- read.table("~/projects/ACE2/FPKM-UQ/note/cart/clinical/denseDataOnlyDownload.tsv",sep="\t",header=T)
head(smo_his)
smo_his2 <- na.omit(smo_his[,c(1,5)])
head(smo_his2)
all_som <- merge(all,na.omit(smo_his[,c(1,5)]),by="sample")
str(all_som)
ggplot(data=subset(all_som,tobacco_smoking_history != 5),
       aes(x=as.character(tobacco_smoking_history),y=log2(FPKM+1)))+geom_boxplot()+
  scale_x_discrete(limits=c("1","3","4","2"),
                   labes=c("Non-smoker","Reformed smoker for <= 15 years",
                           "Reformed smoker for > 15 years","Smoker"))+mytheme_violin
...
nosmok <- as.character(smo_his2$sample[smo_his2$tobacco_smoking_history==1])
head(exposure)       
all1 <- merge(all,exposure[,-1],by="submitter_id")
all1$years_smoked[all1$sample %in% nosmok] <- 0

luad <- subset(all1,project=="LUAD" & years_smoked != "--")
unique(luad$years_smoked)

smo <- luad[,c("FPKM","years_smoked")]
smo$years_smoked <- as.numeric(as.character(smo$years_smoked))
str(smo)

no_smoke <- as.data.frame(all1[all1$sample %in% nosmok,c("FPKM")])

names(no_smoke) <- c("FPKM")
no_smoke$years_smoked <- 0

sm <- rbind(smo,no_smoke)

head(sm)
tail(sm)

cor1 <- cor.test(log2(sm$FPKM+1),sm$years_smoked)
p3 <-ggplot(sm,
            aes(x=years_smoked,y=log2(FPKM+1)))+
  geom_point(size=0.2,colour="#00688B")+mytheme_violin+
  xlab("Years smoked")+
  # geom_abline(intercept=as.numeric(lm$regression.results[2,2]),
  #             slope=as.numeric(lm$regression.results[2,3]),color='brown')+
  annotate("text",size=2.5, x=-Inf, y=Inf,hjust=-.4, vjust=2,label=paste("R = ",signif(cor1$estimate, digit=2),sep=""))+
  annotate("text", size=2.5,x=-Inf, y=Inf,hjust=-.4, vjust=4,label=paste("P = ",signif(cor1$p.value,digit=2),sep=""))+
  annotate("text", size=2.5,x=-Inf, y=Inf,hjust=-.4, vjust=6,label=paste("N = ",length(sm$FPKM),sep=""))
...
ggplot()+geom_histogram(data=NULL,aes(x=as.numeric(all1$years_smoked)))

lm1 <- lmodel2(log2(subset(all1,project=="LUAD" & years_smoked !="--")$FPKM+1)~
                 as.numeric(as.character(subset(all1,project=="LUAD" & years_smoked !="--")$years_smoked)))
lm1
cor1 <- cor.test(log2(subset(all1,project=="LUAD" & years_smoked !="--")$FPKM+1),
                 as.numeric(as.character(subset(all1,project=="LUAD" & years_smoked !="--")$years_smoked)))
cor1
p3 <-ggplot(subset(all1,project=="LUAD" & years_smoked !="--"),
            aes(x=as.numeric(as.character(years_smoked)),y=log2(FPKM+1)))+
  geom_point(size=0.2,colour="#00688B")+mytheme_violin+
  xlab("Years smoked")+
  # geom_abline(intercept=as.numeric(lm$regression.results[2,2]),
  #             slope=as.numeric(lm$regression.results[2,3]),color='brown')+
...
print(p2,vp=viewport(.12,.095,x=.33,y=.9))
print(p3,vp=viewport(.12,.095,x=.2,y=.8))
print(p4,vp=viewport(.12,.095,x=.33,y=.8))
